...
def run_tests():
    ...
    with urllib.request.urlopen(f"{base}/health") as r:
        ...
        print(f"[TEST 1/5] Health Check: Status={health.get('status')}, App={health.get('app_name')}")
    ...
    with urllib.request.urlopen(f"{base}/villages") as r:
        ...
        print(f"[TEST 2/5] Pilot Villages ({len(village_names)} loaded): {village_names}")
    ...
    with urllib.request.urlopen(f"{base}/villages/VIL-01") as r:
        ...
        lead = d["lead_time"]
        print(f"[TEST 3/5] Pandoh Baseline -> Risk: {risk['risk_percentage']}% ({risk['risk_level']}), Model: {risk['model_type']}, Lead Time: {lead['window_display']}")
    ...
    with urllib.request.urlopen(req) as r:
        ...
        print(f"[TEST 4/5] Scenario Applied: {scen.get('scenario')}")
    ...
    with urllib.request.urlopen(f"{base}/villages/VIL-01") as r:
        ...
        print(f"[TEST 5/5] Sensor Outage Fallback Test:")
    ...
    print("=" * 60)
    print("🎉 ALL 5 SYSTEM INTEGRATION TESTS PASSED WITH 100% SUCCESS!")
    print("=" * 60)
